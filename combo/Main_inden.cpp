...
double PerformKernighansShift(const vector< vector<double> >& Q, const vector<double>& correctionVector, const vector<int>& communitiesOld, vector<int>& communitiesNew, const vector<int>& community) //perform a split improvement using a Karnigan-Lin-style iterative shifts series
{
	if(Inden){
		...
		for(size_t i = 0; i < community.size(); ++i){
			if(communitiesOld[i])oldOrig.insert(community[i]);
			else oldDest.insert(community[i]);
		}
		double inden_gain0 = (G.CalcInDen(oldOrig) * oldOrig.size() + G.CalcInDen(oldDest) * oldDest.size()) / n;
		...
		while(g_max1 - g_max > 0){
			...
			vector<int> mv;
			//vector<int> bv;			
			set<int> tempOrig = oldOrig;
			set<int> tempDest = oldDest;

			while(tempOrig.size() > 2 && tempDest.size() > 2){
				vector<double> all_da;
				vector<double> all_db;
				for(set<int>::iterator i = tempOrig.begin(); i != tempOrig.end(); ++i){
					double degree = 0;
					set<int> _temp1 = tempOrig;
					set<int> _temp2 = tempDest;
					_temp1.erase(*i);
					_temp2.insert(*i);
					all_da.push_back((G.CalcInDen(_temp1) * _temp1.size() + G.CalcInDen(_temp2) * _temp2.size()) / n - inden_gain0);
				}
				for(set<int>::iterator i = tempDest.begin(); i != tempDest.end(); ++i){
					double degree = 0;
					set<int> _temp1 = tempOrig;
					set<int> _temp2 = tempDest;
					_temp1.insert(*i);
					_temp2.erase(*i);
					all_db.push_back((G.CalcInDen(_temp1) * _temp1.size() + G.CalcInDen(_temp2) * _temp2.size()) / n - inden_gain0);
				}
				...
				if(*result1 > *result2){
					if(*result1 <= 0)break;
					set<int>::iterator it = tempOrig.begin();
					for(size_t i = 0; i < distance(all_da.begin(), result1); ++i)++it;
					mv.push_back(*it);
					...
					if(*result2 <= 0)break;
					set<int>::iterator it = tempDest.begin();
					for(size_t i = 0; i < distance(all_db.begin(), result2); ++i)++it;
					mv.push_back(*it);
					//cout << "mv = " << mv.back() << endl;
					//cout << communitiesOld[distance(community.begin(), find(community.begin(), community.end(), mv.back()))] << endl;				
			...
			for(size_t i = 0; i < mv.size(); ++i){
				set<int>::iterator it = find(oldOrig.begin(), oldOrig.end(), mv[i]);
				if(it != oldOrig.end()){
					oldOrig.erase(mv[i]);
				}
				else {
		...
		cout << "\n-----oldOrig-----\n";
		for(set<int>::iterator i = oldOrig.begin(); i != oldOrig.end(); ++i){
			if(find(community.begin(), community.end(), *i) == community.end())cerr << "error!\n\n";
			communitiesNew[distance(community.begin(), find(community.begin(), community.end(), *i))] = 1;
		}
		cout << "\n-----oldDest-----\n";
		for(set<int>::iterator i = oldDest.begin(); i != oldDest.end(); ++i){
			if(find(community.begin(), community.end(), *i) == community.end())cerr << "error!\n\n";
			communitiesNew[distance(community.begin(), find(community.begin(), community.end(), *i))] = 0;
		}
		...
		double inden_gain1 = (G.CalcInDen(oldOrig) * oldOrig.size() + G.CalcInDen(oldDest) * oldDest.size()) / n;
		cout << "result111 = " << inden_gain1 - inden_gain0 << endl;
		cout << "inden = " << inden_gain1 << endl;
		return inden_gain1 - inden_gain0;
	}
	else{
		int n = Q.size();
